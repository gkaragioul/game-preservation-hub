...
static qboolean IN_IsCommandShortcut(const SDL_KeyboardEvent* key)
{
	return (key->mod & SDL_KMOD_GUI) != 0;
}

static void IN_ToggleFullscreenShortcut(void)
{
	if (vid_fullscreen == NULL)
		return;

	if (vid_fullscreen->value == 0.0f && vid_mode != NULL)
...
{
	if (!IN_IsCommandShortcut(key))
		return false;

	if (key->key == SDLK_Q)
		return true;

	return (key->key == SDLK_F4 || key->scancode == SDL_SCANCODE_F4);
}

static qboolean IN_IsBenchmarkOverlayShortcut(const SDL_KeyboardEvent* key)
{
	if (key->key == SDLK_F10 || key->scancode == SDL_SCANCODE_F10)
		return true;

	if (!IN_IsCommandShortcut(key))
		return false;

	return (key->key == SDLK_B || key->scancode == SDL_SCANCODE_B);
}
...
void IN_Init(void)
{
	IN_InitMouse();
	IN_InitController();
	vid_windowed_mode = Cvar_Get("vid_windowed_mode", "1", CVAR_ARCHIVE);
		...
		switch (event.type)
		{
			...
			case SDL_EVENT_KEY_UP:
			{
				...
				const SDL_Keycode kc = event.key.key;
					...
					if (down && !event.key.repeat && IN_IsCommandShortcut(&event.key))
					{
						if (IN_IsForceQuitShortcut(&event.key))
							Sys_Quit();
						...
						if (IN_IsBenchmarkOverlayShortcut(&event.key))
						{
							Cbuf_AddText("perf_overlay\n");
							break;
						}
					}

					if (down && !event.key.repeat && IN_IsBenchmarkOverlayShortcut(&event.key))
					{
						Cbuf_AddText("perf_overlay\n");
						break;
					}
