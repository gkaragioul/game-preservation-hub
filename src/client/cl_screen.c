...
static cvar_t* scr_perf_overlay;
...
typedef struct perf_snapshot_s
{
	float fps;
	float frame_ms;
	float gpu_ms;
	float cpu_pct;
	float ram_process_mb;
	float ram_used_gb;
	float ram_total_gb;
	int health_pct;
	qboolean system_stats_available;
} perf_snapshot_t;

static perf_snapshot_t perf_snapshot;
static int perf_snapshot_time;
...
void SCR_Init(void)
{
	...
	scr_perf_overlay = Cvar_Get("scr_perf_overlay", "0", CVAR_ARCHIVE);
	...
	Cmd_AddCommand("perf_overlay", SCR_PerfOverlay_f);
...
void SCR_WindowResized(void)
{
	SCR_UpdateUIScale();

	if (!scr_initialized || scr_viewsize == NULL)
		return;
...
static double SCR_GetProcessCPUSeconds(void)
{
#ifdef __APPLE__
	struct rusage usage;
...
static qboolean SCR_GetMemoryStats(float* process_mb, float* used_gb, float* total_gb)
{
#ifdef __APPLE__
	...
	if (sysctlbyname("hw.memsize", &total_bytes, &total_size, NULL, 0) != 0 || total_bytes == 0)
		return false;

	vm_statistics64_data_t vm_stats;
	mach_msg_type_number_t vm_stats_count = HOST_VM_INFO64_COUNT;

	if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vm_stats, &vm_stats_count) != KERN_SUCCESS)
		return false;

	vm_size_t page_size = 0;
	host_page_size(mach_host_self(), &page_size);

	const uint64_t free_bytes = (uint64_t)(vm_stats.free_count + vm_stats.speculative_count) * (uint64_t)page_size;
	const uint64_t used_bytes = (total_bytes > free_bytes ? total_bytes - free_bytes : 0);

	*process_mb = (float)((double)task_info_data.resident_size / (1024.0 * 1024.0));
	*used_gb = (float)((double)used_bytes / (1024.0 * 1024.0 * 1024.0));
	*total_gb = (float)((double)total_bytes / (1024.0 * 1024.0 * 1024.0));
...
static const char* SCR_GetPerfHealthLabel(const int health_pct)
{
	if (health_pct >= 95)
		return "Excellent";
	if (health_pct >= 80)
...
static void SCR_UpdatePerfSnapshot(void)
{
	const int interval_ms = max(100, (int)(scr_perf_overlay_interval->value * 1000.0f));

	if (perf_snapshot_time != 0 && cls.realtime - perf_snapshot_time < interval_ms)
		return;

	const float frame_seconds = max(cls.rframetime, 0.0001f);
	const float target_fps = max(vid_maxfps->value, 1.0f);
	const float frame_budget_ms = 1000.0f / target_fps;

	perf_snapshot.fps = 1.0f / frame_seconds;
	perf_snapshot.frame_ms = frame_seconds * 1000.0f;
	perf_snapshot.gpu_ms = Cvar_VariableValue("r_gpu_frame_ms");
	perf_snapshot.health_pct = (int)Clamp((frame_budget_ms / max(perf_snapshot.frame_ms, 0.001f)) * 100.0f, 0.0f, 100.0f);

	const double cpu_seconds = SCR_GetProcessCPUSeconds();
	if (perf_cpu_last_time != 0 && cls.realtime > perf_cpu_last_time)
	{
		const double wall_seconds = (double)(cls.realtime - perf_cpu_last_time) / 1000.0;
		const double cpu_delta = max(0.0, cpu_seconds - perf_cpu_last_seconds);

		perf_snapshot.cpu_pct = (float)Clamp((cpu_delta / wall_seconds) * 100.0, 0.0, 999.0);
	}

	perf_cpu_last_time = cls.realtime;
	perf_cpu_last_seconds = cpu_seconds;
	perf_snapshot.system_stats_available = SCR_GetMemoryStats(&perf_snapshot.ram_process_mb, &perf_snapshot.ram_used_gb, &perf_snapshot.ram_total_gb);
...
static void SCR_LogFrameSpike(void)
{
	if ((int)scr_frame_spike_log->value == 0 || vid_maxfps == NULL)
		return;

	const float target_fps = max(vid_maxfps->value, 1.0f);
	const float threshold = Clamp(scr_frame_spike_threshold->value, 0.1f, 1.0f);
	const float spike_ms = 1000.0f / (target_fps * threshold);
	const float frame_ms = max(cls.rframetime, 0.0001f) * 1000.0f;

	if (frame_ms < spike_ms)
		return;

	if (perf_last_spike_log_time != 0 && cls.realtime - perf_last_spike_log_time < 500)
		return;

	perf_last_spike_log_time = cls.realtime;

	char line[512];
	Com_sprintf(
		line,
		sizeof(line),
		"FRAME SPIKE: frame %.2f ms (%.0f FPS target, %.0f%% threshold), gpu %.2f ms, ent %i alpha %i lights %i particles %i+%i, bloom %.0f ssao %.0f shadows %.0f reflections %.0f detail %.0f vsync %.0f\n",
		frame_ms,
		target_fps,
		threshold * 100.0f,
	...
	char log_path[MAX_OSPATH] = "frame_spikes.log";
	...
	if (home != NULL && home[0] != '\0')
	{
		...
		mkdir(log_dir, 0755);
		Com_sprintf(log_path, sizeof(log_path), "%s/frame_spikes.log", log_dir);
	}
#endif
...
static const byte* SCR_GetPerfGlyph(char c)
{
	static const byte g_space[] = {0,0,0,0,0,0,0};
	static const byte g_qmark[] = {14,17,1,2,4,0,4};
	static const byte g_a[] = {14,17,17,31,17,17,17};
	static const byte g_b[] = {30,17,17,30,17,17,30};
	static const byte g_c[] = {15,16,16,16,16,16,15};
	static const byte g_d[] = {30,17,17,17,17,17,30};
	static const byte g_e[] = {31,16,16,30,16,16,31};
	static const byte g_f[] = {31,16,16,30,16,16,16};
	static const byte g_g[] = {15,16,16,23,17,17,15};
	static const byte g_h[] = {17,17,17,31,17,17,17};
	static const byte g_i[] = {31,4,4,4,4,4,31};
	static const byte g_j[] = {7,2,2,2,2,18,12};
	static const byte g_k[] = {17,18,20,24,20,18,17};
	static const byte g_l[] = {16,16,16,16,16,16,31};
	static const byte g_m[] = {17,27,21,21,17,17,17};
	static const byte g_n[] = {17,25,21,19,17,17,17};
	static const byte g_o[] = {14,17,17,17,17,17,14};
	static const byte g_p[] = {30,17,17,30,16,16,16};
	static const byte g_q[] = {14,17,17,17,21,18,13};
	static const byte g_r[] = {30,17,17,30,20,18,17};
	static const byte g_s[] = {15,16,16,14,1,1,30};
	static const byte g_t[] = {31,4,4,4,4,4,4};
	static const byte g_u[] = {17,17,17,17,17,17,14};
	static const byte g_v[] = {17,17,17,17,17,10,4};
	static const byte g_w[] = {17,17,17,21,21,21,10};
	static const byte g_x[] = {17,17,10,4,10,17,17};
	static const byte g_y[] = {17,17,10,4,4,4,4};
	static const byte g_z[] = {31,1,2,4,8,16,31};
	static const byte g_0[] = {14,17,19,21,25,17,14};
	static const byte g_1[] = {4,12,4,4,4,4,14};
	static const byte g_2[] = {14,17,1,2,4,8,31};
	static const byte g_3[] = {30,1,1,14,1,1,30};
	static const byte g_4[] = {2,6,10,18,31,2,2};
	static const byte g_5[] = {31,16,16,30,1,1,30};
	static const byte g_6[] = {14,16,16,30,17,17,14};
	static const byte g_7[] = {31,1,2,4,8,8,8};
	static const byte g_8[] = {14,17,17,14,17,17,14};
	static const byte g_9[] = {14,17,17,15,1,1,14};
	static const byte g_colon[] = {0,4,4,0,4,4,0};
	static const byte g_dot[] = {0,0,0,0,0,12,12};
	static const byte g_dash[] = {0,0,0,31,0,0,0};
	static const byte g_slash[] = {1,2,2,4,8,8,16};
	static const byte g_percent[] = {25,26,2,4,8,11,19};
	static const byte g_plus[] = {0,4,4,31,4,4,0};
	...
	switch (c)
	{
		...
		case ':': return g_colon; case '.': return g_dot; case '-': return g_dash; case '/': return g_slash;
		case '%': return g_percent; case '+': return g_plus;
		default: return g_qmark;
	}
}

static int SCR_PerfFontScale(void)
{
	return max(1, (ui_scale * 7 + 5) / 10);
}

static int SCR_PerfLineHeight(void)
{
	return 8 * SCR_PerfFontScale();
}

static int SCR_PerfTextWidth(const char* text)
{
	return (int)strlen(text) * 6 * SCR_PerfFontScale();
}

static void SCR_AddPerfLine(char lines[][128], paletteRGBA_t colors[], int* count, const char* text, const paletteRGBA_t color)
{
	if (*count >= 12)
		return;

	strcpy_s(lines[*count], 128, text);
	colors[*count] = color;
	(*count)++;
}

static void SCR_DrawPerfLine(int x, int* y, const char* text, paletteRGBA_t color)
{
	const int scale = SCR_PerfFontScale();

	for (const char* s = text; *s != '\0'; s++, x += 6 * scale)
	{
		const byte* glyph = SCR_GetPerfGlyph(*s);
		...
		for (int row = 0; row < 7; row++)
		{
			...
			{
				const qboolean filled = (col < 5 && (glyph[row] & (1 << (4 - col))) != 0);

				if (filled && run_start < 0)
					run_start = col;
				else if (!filled && run_start >= 0)
				{
					re.DrawFill(x + run_start * scale, *y + row * scale, (col - run_start) * scale, scale, color);
					run_start = -1;
				}
			}
		}
	...
	*y += SCR_PerfLineHeight();
}

static void SCR_DrawPerfOverlay(void)
{
	if ((int)scr_perf_overlay->value == 0)
		return;

	SCR_UpdatePerfSnapshot();

	char line[128];
	char lines[12][128];
	paletteRGBA_t colors[12];
	int line_count = 0;

	SCR_AddPerfLine(lines, colors, &line_count, "BENCHMARK", TextPalette[P_YELLOW]);

	Com_sprintf(line, sizeof(line), "FPS: %5.1f   Frame: %4.1f ms", perf_snapshot.fps, perf_snapshot.frame_ms);
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

	if (perf_snapshot.gpu_ms > 0.0f)
		Com_sprintf(line, sizeof(line), "GPU frame: %4.1f ms", perf_snapshot.gpu_ms);
	else
		Com_sprintf(line, sizeof(line), "GPU frame: n/a");
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

	Com_sprintf(line, sizeof(line), "GPU: %.32s", Cvar_VariableString("r_gpu_renderer"));
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

	if (perf_snapshot.system_stats_available)
	{
		Com_sprintf(line, sizeof(line), "CPU: %4.0f%% process", perf_snapshot.cpu_pct);
		SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

		Com_sprintf(line, sizeof(line), "RAM: %.0f MB app", perf_snapshot.ram_process_mb);
		SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

		Com_sprintf(line, sizeof(line), "System RAM: %.1f / %.1f GB", perf_snapshot.ram_used_gb, perf_snapshot.ram_total_gb);
		SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);
	}
	else
	{
		SCR_AddPerfLine(lines, colors, &line_count, "CPU/RAM: n/a", TextPalette[P_WHITE]);
	}

	Com_sprintf(line, sizeof(line), "Render: %i ent  %i alpha  %i lights", cl.refdef.num_entities, cl.refdef.num_alpha_entities, cl.refdef.num_dlights);
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

	Com_sprintf(line, sizeof(line), "Particles: %i + %i", cl.refdef.num_particles, cl.refdef.anum_particles);
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_WHITE]);

	Com_sprintf(line, sizeof(line), "Perf: %i%% of %.0f FPS target", perf_snapshot.health_pct, vid_maxfps->value);
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_YELLOW]);

	Com_sprintf(line, sizeof(line), "Overall: %s", SCR_GetPerfHealthLabel(perf_snapshot.health_pct));
	SCR_AddPerfLine(lines, colors, &line_count, line, TextPalette[P_YELLOW]);

	int max_width = 0;
	for (int i = 0; i < line_count; i++)
		max_width = max(max_width, SCR_PerfTextWidth(lines[i]));

	const int scale = SCR_PerfFontScale();
	const int margin = max(12, 12 * scale);
	const int padding = max(3, 4 * scale);
	const int x = margin;
	int y = margin;
	const int panel_width = max_width + 2 * padding;
	const int panel_height = line_count * SCR_PerfLineHeight() + 2 * padding;

	paletteRGBA_t bg = TextPalette[P_BLACK];
	bg.a = 180;
	re.DrawFill(x - padding, y - padding, panel_width, panel_height, bg);

	for (int i = 0; i < line_count; i++)
		SCR_DrawPerfLine(x, &y, lines[i], colors[i]);
}
