...
void Menu_DrawBG(const char* bk_path, const float scale) //mxd
{
	...
	// Use the widescreen book for standard conback menus when display is wider than 4:3.
	const qboolean is_widescreen = ((float)viddef.width * 0.75f > (float)viddef.height);
	m_widescreen_book = (is_widescreen && strstr(bk_path, "conback") != NULL);
	...
	{
		const float book_aspect = 16.0f / 9.0f;
		int book_w = viddef.width;
		int book_h = (int)((float)book_w / book_aspect + 0.5f);

		if (book_h > viddef.height)
		{
			book_h = viddef.height;
			book_w = (int)((float)book_h * book_aspect + 0.5f);
		}

		const int ox = (viddef.width - book_w) / 2;
		const int oy = (viddef.height - book_h) / 2;
		re.DrawStretchPic(ox, oy, book_w, book_h, "/Book/Back/WidescreenBook.m32", 1.0f, DSP_NONE);
	}
}
