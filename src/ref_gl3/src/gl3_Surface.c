...
#define GL3_STACK_POLY_VERTS 64
#define GL3_MAX_SAFE_POLY_VERTS 1024

static qboolean R_IsValidPoly(const glpoly_t* p)
{
...
static void R_DrawAlphaSurface(const msurface_t* fa)
{
	...
	else
	{
		...
		if (R_IsValidPoly(p))
		{
			float vbuf[GL3_STACK_POLY_VERTS * 9];
			const int numverts = min(p->numverts, GL3_STACK_POLY_VERTS);
			...
			GL3_Draw3DPoly(GL_TRIANGLE_FAN, vbuf, numverts);
		}
	}
...
static void R_RenderLightmappedPoly(const entity_t* ent, msurface_t* surf)
{
	...
	if (surf == NULL || surf->texinfo == NULL || surf->polys == NULL)
		return;
	...
	if (is_dynamic)
	{
		...
		const byte dynamic_style = (map < MAXLIGHTMAPS ? surf->styles[map] : 255);

		if (smax <= 0 || tmax <= 0 || smax > LM_BLOCK_WIDTH || tmax > LM_BLOCK_HEIGHT)
			return;

		R_BuildLightMap(surf, (byte*)lightmap_pixels, smax * 4);

		if ((dynamic_style >= 32 || dynamic_style == 0) && surf->dlightframe != r_framecount)
		{
			if (surf->lightmaptexturenum < 0 || surf->lightmaptexturenum >= MAX_LIGHTMAPS)
				return;
	...
	if (lmtex < 0 || lmtex >= MAX_LIGHTMAPS + 1)
		return;
...
static void R_RenderBrushPoly(const entity_t* ent, msurface_t* fa)
{
	...
	if (fa->dlightframe == r_framecount || is_dynamic)
	{
		...
		{
			const byte dynamic_style = (map < MAXLIGHTMAPS ? fa->styles[map] : 255);

			if ((dynamic_style >= 32 || dynamic_style == 0) && fa->dlightframe != r_framecount)
			{
				uint temp[34 * 34];
