...
static int ClampFrameIndex(const fmdl_t* fmdl, const int frame)
{
	return (frame >= 0 && frame < fmdl->header.num_frames ? frame : 0);
}
...
static void LerpStandardSkeleton(const fmdl_t* fmdl, entity_t* e) //mxd. Original logic uses 'fmodel' and 'currententity' global vars.
{
	...
	if (e->swapFrame != NO_SWAP_FRAME)
	{
		...
		const float lerp_val = 1.0f - e->backlerp; //mxd

		for (int i = 0; i < 3; i++)
		{
			swap_skel_move[i] = lerp_val * pframe->translate[i] + e->backlerp * poldframe->translate[i];
			sfl_swap_skel.front_vector[i] = lerp_val * pframe->scale[i];
...
static void StandardFrameLerp(const fmdl_t* fmdl, entity_t* e) //mxd. Original logic uses 'fmodel' and 'currententity' global vars.
{
	...
	frame = ClampFrameIndex(fmdl, frame);
	oldframe = ClampFrameIndex(fmdl, oldframe);
	...
	if (e->swapFrame != NO_SWAP_FRAME)
	{
		const int swap_frame = ClampFrameIndex(fmdl, e->swapFrame);
		const int old_swap_frame = ClampFrameIndex(fmdl, e->oldSwapFrame);

		if (swap_frame == 0 && e->swapFrame != 0)
		{
			e->swapFrame = NO_SWAP_FRAME;
			e->oldSwapFrame = NO_SWAP_FRAME;
			e->swapCluster = 0;
		}
		else
		{
			e->swapFrame = swap_frame;
			e->oldSwapFrame = old_swap_frame;
		}
	}
	...
	for (int i = 0; i < 3; i++)
	{
		cur_skel_move[i] = lerp_val * pframe->translate[i] + e->backlerp * poldframe->translate[i];
		sfl_cur_skel.front_vector[i] = lerp_val * pframe->scale[i];
