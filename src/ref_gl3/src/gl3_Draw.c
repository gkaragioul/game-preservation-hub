...
void Draw_BookPic(const char* name, const float scale, const float alpha)
{
	...
	if (header_w / header_h <= 4.0f / 3.0f + 0.01f) //mxd. Preserve standard 4:3 book assets; skip for widescreen book assets.
	{
		if (vid_w * 0.75f > vid_h) // Wide window: pillarbox the 4:3 book.
		{
			vid_w = vid_h * 4 / 3;
			offset_x = (viddef.width - (int)vid_w) / 2;
		}
		else if (vid_w * 0.75f < vid_h) // Tall/narrow window: letterbox and keep the book centered.
		{
			vid_h = vid_w * 3 / 4;
			offset_y = (viddef.height - (int)vid_h) / 2;
		}
	}
