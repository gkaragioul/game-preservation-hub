...
{
	if (sdl_stream == NULL || length < 1)
		return;

	if (sound.buffer == NULL || samplesize < 1 || sound.channels < 1 || sound.samplebits < 1)
	{
		memset(sdl_stream, 0, length);
		return;
	}

	const int bytes_per_sample = sound.samplebits / 8;
	...
	int pos = start_sample * bytes_per_sample;
	const int to_buffer_end = samplesize - pos;

	int length1;
	int length2;

	if (copy_length > to_buffer_end)
	{
		length1 = to_buffer_end;
		length2 = copy_length - length1;
	}
	else
	{
		length1 = copy_length;
	...
	{
		memset(sdl_stream + copy_length, 0, length - copy_length);

		if (requested_frames > available_frames)
		{
			audio_underruns++;
			if ((audio_underruns & 31) == 1)
				si.Com_DPrintf("SDL audio underrun: needed %i frames, had %i\n", requested_frames, available_frames);
		}
	}
