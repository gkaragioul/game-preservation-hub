...
- `addons/` - maps, textures and text files from third-party fan add-ons. Their sounds, models and videos are not included; see [Third-party add-ons](#third-party-add-ons).
...
The folders in `addons/` come from fan-made Heretic II add-ons released between 1999 and 2003. They belong to their original authors, whose own readme files are kept in each folder. They are not covered by this repository's licence.

Their sound, model and video files are not included, because some of them were taken from commercial games and films. The maps may therefore be missing sounds or models. Two add-ons, Golgotha and Turtle, are not included at all, because their authors only allow them to be shared complete and unaltered. To play an add-on as its author intended, get the complete original release from a Heretic II fan archive.

If you are an author of one of these add-ons and want your files removed, open an issue in this hub.

## Third-party code

Libraries bundled with or linked by this port (SDL3, OpenAL Soft, libsmacker, glad, stb) keep their own licences. They are listed in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
...
This project is not affiliated with or endorsed by Raven Software, Activision or id Software. Heretic II is a trademark of its owners.
