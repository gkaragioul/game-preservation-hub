...
#define GL3_WARP_STACK_VERTS 64

static qboolean R_IsValidWarpPoly(const glpoly_t* p)
{
	return p != NULL && p->numverts >= 3;
}
...
void R_EmitWaterPolys(const msurface_t* fa, const qboolean undulate, const qboolean use_reflect)
{
	...
	{
		if (!R_IsValidWarpPoly(p))
			continue;

		float vbuf[GL3_WARP_STACK_VERTS * 9];
		const int numverts = min(p->numverts, GL3_WARP_STACK_VERTS);
		...
		if (use_reflect)
			GL3_DrawWaterPoly(GL_TRIANGLE_FAN, vbuf, numverts);
		else
