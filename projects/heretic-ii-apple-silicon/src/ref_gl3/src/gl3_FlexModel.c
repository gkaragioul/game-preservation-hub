//
// gl3_FlexModel.c -- Heretic 2 FlexModel loading and rendering for GL3 Core Profile.
//
// Based on gl1_FlexModel.c -- Copyright 1998 Raven Software
//
...
static qboolean fmLoadHeader(fmdl_t* fmdl, model_t* model, const int version, const int datasize, const void* buffer)
{
    ...
    if (h->skinwidth < 1 || h->skinwidth > SKINPAGE_WIDTH || h->skinheight < 1 || h->skinheight > SKINPAGE_HEIGHT)
    ...
    if (h->num_tris < 1 || h->num_tris > MAX_FM_TRIANGLES)
        ri.Sys_Error(ERR_DROP, "Model '%s' has invalid number of triangles (%i)", model->name, h->num_tris);

    if (h->num_frames < 1 || h->num_frames > MAX_FM_FRAMES)
...
// FM_SHORT_FRAME, FM_NORMAL and FM_COMP blocks are never used in any H2 models.
...
static qboolean fmLoadSkeleton(fmdl_t* fmdl, model_t* model, const int version, const int datasize, const void* buffer)
{
    ...
    if (fmdl->skeletalType < 0 || fmdl->skeletalType >= NUM_SKELETONS)
    {
        ri.Con_Printf(PRINT_ALL, "Invalid skeletal type for model '%s': %d\n", model->name, fmdl->skeletalType);
        return false;
    }

    const int num_clusters = *(++in_i);
    const int num_joints = numJointsInSkeleton[fmdl->skeletalType];

    fmdl->rootCluster = CreateSkeletonForModel(fmdl->skeletalType, num_clusters);
    if (fmdl->rootCluster == -1)
    {
        ri.Con_Printf(PRINT_ALL, "No room for skeleton clusters in model '%s'\n", model->name);
        return false;
    }
    ...
    if (have_skeleton)
    {
        ...
        for (int i = 0; i < fmdl->header.num_frames; i++)
        {
            ...
            {
                const float origin[3] = { *(++in_f), *(++in_f), *(++in_f) };
                const float direction[3] = { *(++in_f), *(++in_f), *(++in_f) };
                const float up[3] = { *(++in_f), *(++in_f), *(++in_f) };

                if (c >= num_joints)
                    continue;

                VectorCopy(origin, fmdl->skeletons[i].rootJoint[c].model.origin);
                VectorCopy(direction, fmdl->skeletons[i].rootJoint[c].model.direction);
                VectorCopy(up, fmdl->skeletons[i].rootJoint[c].model.up);
...
void Mod_LoadFlexModel(model_t* mod, void* buffer, int length)
{
    ...
    while (length > 0)
    {
        ...
        for (loader = &fmblocks[0]; loader->ident[0] != 0; loader++)
        {
            ...
            {
                if (!loader->load(fmdl, mod, header->version, header->size, in))
                {
...
static image_t* R_GetSkin(const entity_t* ent)
{
    if (ent->skin != NULL)
        return ent->skin;
...
static image_t* R_GetSkinFromNode(const entity_t* ent, const int skin_index)
{
    image_t* skin;
    const model_t* mdl = *ent->model;
    ...
    for (int i = 0; i < fmdl->header.num_mesh_nodes; i++, nodeinfo++)
    {
        ...
        while (true)
        {
            ...
            for (int c = 0; c < num_verts; c++)
            {
                ...
                if (use_color)
                {
                    v[5] = (float)nodeinfo->color.r / 255.0f;
                    v[6] = (float)nodeinfo->color.g / 255.0f;
                    v[7] = (float)nodeinfo->color.b / 255.0f;
                    v[8] = (float)nodeinfo->color.a / 255.0f;
                }
...
{
    const fmdl_t* fmdl = (fmdl_t*)(*e->model)->extradata;

    if (R_CullFlexModel(fmdl, e))
        return;

    vec3_t shadelight;
    qboolean apply_minlight = false;

    if (e->flags & RF_TRANS_ADD_ALPHA)
    {
        const float a = (float)e->color.a / 255.0f;
        VectorSet(shadelight, a, a, a);
    }
    else if (e->flags & RF_FULLBRIGHT)
    {
    ...
    {
        R_LightPoint(e->origin, shadelight, true);
        apply_minlight = true;
    }
    ...
    if (apply_minlight && gl_state.minlight_set)
    {
        for (int i = 0; i < 3; i++)
        {
            const int lv = (int)(shadelight[i] * 255.0f);
            shadelight[i] = (float)minlight[min(255, lv)] / 255.0f;
        }
    }
    ...
    if (e->flags & RF_DEPTHHACK)
        glDepthRange((double)gldepthmin, (double)((gldepthmax - gldepthmin) * 0.3f + gldepthmin));
