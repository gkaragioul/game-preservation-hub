...
void CreateSkeletonAsHunk(const int structure, ModelSkeleton_t* skel)
{
	...
	memset(skel->rootJoint, 0, numJointsInSkeleton[structure] * sizeof(M_SkeletalJoint_t));
	memset(skel->rootNode, 0, numNodesInSkeleton[structure] * sizeof(ArrayedListNode_t));
...
int CreateSkeletonForModel(const int structure, const int num_clusters)
{
	const int allocation_size = (num_clusters > numJointsInSkeleton[structure] ? num_clusters : numJointsInSkeleton[structure]);
	const int index = GetRootIndex(MAX_ARRAYED_SKELETAL_JOINTS, allocation_size);
...
void LinearlyInterpolateJoints(ModelSkeleton_t* newSkel, const int newIndex, ModelSkeleton_t* oldSkel, const int oldIndex, ModelSkeleton_t* liSkel, const int liIndex, float move[3], float frontv[3], float backv[3]) //mxd. Named 'LinearllyInterpolateJoints' in original logic.
{
	...
	if (newJoint->children != ARRAYEDLISTNODE_NULL)
	{
		...
		for (newChild = newJoint->children, oldChild = oldJoint->children, liChild = liJoint->children;
			 newChild != ARRAYEDLISTNODE_NULL && oldChild != ARRAYEDLISTNODE_NULL && liChild != ARRAYEDLISTNODE_NULL;
