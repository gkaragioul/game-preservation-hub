...
static qboolean R_IsAdditiveParticleAtlas(const image_t* image)
{
...
static void R_UploadAdditiveParticleAtlas(const int level, const byte* data, const paletteRGB_t* palette, const int width, const int height)
{
	...
	if (dst_size > upload_buffer_size)
	{
		...
		if (upload_buffer == NULL)
			ri.Sys_Error(ERR_DROP, "R_UploadAdditiveParticleAtlas: failed to allocate upload buffer for %i x %i image!\n", width, height);
...
image_t* R_FindImage(const char* name, const imagetype_t type)
{
	...
	if ((int)r_hd_textures->value || Q_stricmp(name, "book/Back/WidescreenBook.m32") == 0)
