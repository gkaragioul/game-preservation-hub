...
static menuaction_t s_addons_action;
...
static void AddonsGameFunc(void* data)
{
	M_Menu_Addons_f();
}
...
static void Game_MenuInit(void)
{
	...
	static char name_addons[MAX_QPATH];
	...
	Com_sprintf(name_addons, sizeof(name_addons), "\x02%s", "Game Add-ons");
	s_addons_action.generic.type = MTYPE_ACTION;
	s_addons_action.generic.flags = QMF_LEFT_JUSTIFY | QMF_SELECT_SOUND;
	s_addons_action.generic.x = 0;
	s_addons_action.generic.y = 200;
	s_addons_action.generic.name = name_addons;
	s_addons_action.generic.width = re.BF_Strlen(name_addons);
	s_addons_action.generic.callback = AddonsGameFunc;
	...
	Menu_AddItem(&s_game_menu, &s_addons_action);
