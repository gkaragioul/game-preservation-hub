    ...
    try:
        while True:
            ...
            for k, v in headers.items():
                vv = v
                if any(x in k for x in ("auth", "token", "cookie")):
                    vv = v[:24] + "...(redacted)" if len(v) > 24 else v
                blk.append(f"    {k}: {vv}")
            ...
            log_file(host, "\n".join(rblk))
            # structured, correctly-paired capture for building the replay map
            try:
